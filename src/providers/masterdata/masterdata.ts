...
export class MasterdataProvider {
  ...
  postDocument_ScanStatus(StepCurr, StepNext, jsnData, imgSignature, UserScan): Promise<any> {
    ...
    return this.apiProvider.postApiEndpoint(sFile_Handler, JSON.stringify({ CurrStep: StepCurr, NextStep: StepNext, sImg: imgSignature, sUserID: UserScan, sAssignUserID: UserScan, itm: jsnData }), imgSignature);
